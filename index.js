...
`
	...
	// Inject CSS styles into the page head
	...
	tableHeader.prepend('<td class="z"></td>')
	tableHeader.prepend('<td class="z"></td>')
	
	// Process each row in the table (excluding the header row)
	table.find('tr').not(tableHeader).each((i, row) => {
...
const processDetailsPage = async () => {
	...
	downloadCell.insertAdjacentHTML("beforebegin", `<td style="width: 30px;"><a title="Magnet-ссылка" href="magnet:?xt=urn:btih:${torrentHash}" style="display: block; font-size: 0;">${magnetIcon}</a></td>`);
}
